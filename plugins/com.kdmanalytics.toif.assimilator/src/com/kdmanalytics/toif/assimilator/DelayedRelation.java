...
class DelayedRelation {
   ...
   */
  public void commit(PrintWriter out, Repository repository, RepositoryConnection con, XMLNode root,
                     Map<String, String> nodeNames) throws RepositoryException {
    if (debug) {
      System.err.println("============================================");
    ...
    if (object == null) {
      for (StringTokenizer st = new StringTokenizer(snk); st.hasMoreTokens();) {
        ...
        if (target == null) {
          ...
          if (root != null) target = root.getURIByPath(token);
          if (target == null) {
            LOG.error("Cannot commit delayed relation " + this);
            continue;
          }
        ...
        if (debug) {
          System.err.println("adding statement... subject:" + subject + " predicate: " + predicate + " object: "
                             + object);
