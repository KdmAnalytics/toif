...
final public class BuildInformation
	...
	private String KDMVersionFormat( String versionString)
		...
		// Check if KDM format
		if (!versionString.trim().matches("[0-9].[0-9].[0-9].*.\\([0-9]*-.*.\\)"))
			return versionString;
		else
			{
			...
			if (token.length == 2)
				...
				if (buildNumber.contains("-") && buildNumber.length() > 2)
				    buildNumber = token[1].substring(1, token[1].indexOf("-"));
			
				// Generate KDM version string
				if (versionString.contains("SNAPSHOT"))
					return ( version + '-' + buildNumber + " (development)");
				else
					return ( version + '-' + buildNumber + " (production)");
				}
	...
	public boolean isKdmEntity()
		...
		if (vendor != null)
			if (vendor.contains("KDM Analytics"))
				return true;
		
		vendor = attributes.getValue("Bundle-Vendor");
		if (vendor != null)
			if (vendor.contains("KDM Analytics") ||  vendor.contains("KDMANALYTICS"))
				return true;
