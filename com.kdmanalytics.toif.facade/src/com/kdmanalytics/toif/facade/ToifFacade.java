...
public class ToifFacade implements IToifFacade
{
    ...
    public boolean isAdapterReportingToolRunnable(AbstractAdaptor adaptor)
    {
        try
        {
            ...
            if ("findbugs".equals(runtimeName))
            {
                ...
                {
                    process = new ProcessBuilder();
                    String[] commands = { "cmd.exe", "/C", "findbugs.bat", "-version" };
                    process.command(commands);
     ...
     */
    @Override
   	public boolean execute(AbstractAdaptor adaptor, File inputFile,
   			File houseKeepingFile, File outputDirectory, File workingDirectory,
   			String[] additionalArgs,  String rename)
   			throws IllegalArgumentException, TOIFExecuteException
   		{
   		return execute( adaptor, inputFile, houseKeepingFile, outputDirectory,
   				        workingDirectory, additionalArgs, null, rename);
   		}
   	
    
    @Override
    public boolean execute(AbstractAdaptor adaptor, File inputFile, File houseKeepingFile, File OutputDirectory, File workingDirectory,
            String additionalArgs[], boolean[] validLines, String rename) throws IllegalArgumentException, TOIFExecuteException
    {
    ...
    checkNotNull( OutputDirectory, "output directory must be specified");
        ...
        List<String> args = Lists.newArrayList("-i", inputFile.getAbsolutePath(), "-h", houseKeepingFile.getAbsolutePath(), "-o",
                OutputDirectory.getAbsolutePath(), "--unknownCWE");
    ...
    @Override
    public boolean merge(File kdmOutputFile, Set<File> toifFiles, boolean createZip) throws TOIFExecuteException
    {
	...
	@Override
	public boolean merge(File kdmOutputFile, File kdmFile, Set<File> toifFiles,
			boolean createZip) throws TOIFExecuteException
		{
		 Assimilator ass = new Assimilator(createZip);
