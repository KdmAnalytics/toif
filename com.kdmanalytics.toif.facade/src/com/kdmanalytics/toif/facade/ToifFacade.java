...
public class ToifFacade implements IToifFacade
	...
	public Set<AbstractAdaptor> runnableAdapterReportingTool()
		...
		for (AbstractAdaptor instance : ToolAdaptorUtil.getAdaptors())
			...
			try
				...
				{
				System.err.println("The process for "
						+ instance.getAdaptorName() + " could not be started.");
				continue;
				}
			}
	...
	public boolean isAdapterReportingToolRunnable(AbstractAdaptor adaptor)
		...
		try
			...
			{
			System.err.println("The process for " + adaptor.getAdaptorName()
	...
	public boolean execute(AbstractAdaptor adaptor, File inputFile,
		...
		try
			{
			return execute(adaptor, inputFile, houseKeepingFile,
					outputDirectory, workingDirectory, additionalArgs, null,
					rename);
			}
	...
	public boolean execute(AbstractAdaptor adaptor, File inputFile,
			File houseKeepingFile, File OutputDirectory, File workingDirectory,
			String additionalArgs[], boolean[] validLines, String rename)
			throws IllegalArgumentException, ToifException
		{
		...
		List<String> args = Lists.newArrayList("-i",
				inputFile.getAbsolutePath(), "-h",
				houseKeepingFile.getAbsolutePath(), "-o",
		...
		try
			...
			return toolAdaptor.runToolAdaptor(adaptor, args, workingDirectory,
					validLines);
			}
		// do not map IllegalArgumentException
		catch( IllegalArgumentException iex)
	...
	@Override
	public boolean merge(File kdmOutputFile, Set<File> toifFiles,
			boolean createZip) throws ToifException
		{
		// Provide exception guard for internal TOIF Exceptions
		try
			{
			Assimilator ass = new Assimilator(createZip);
			List<String> args = Lists.newArrayList("-k",
					kdmOutputFile.getAbsolutePath());
	...
	public boolean merge(File kdmOutputFile, File kdmFile, Set<File> toifFiles,
		...
		try
			{
			Assimilator ass = new Assimilator(createZip);
			List<String> args = Lists.newArrayList("-m",
					kdmOutputFile.getAbsolutePath());
