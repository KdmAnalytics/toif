...
public class CppcheckAdaptor extends AbstractAdaptor
{
    ...
    public String[] runToolCommands(AdaptorOptions options, String[] otherOpts)
    {
        ...
        final String[] commands = { "cppcheck", "--enable=all", "--xml", "-q", options.getInputFile().toString() };
    ...
    /**
     * parse the output of the tool. In this case, because cppcheck outputs xml
     * on the error stream, I am using an XMLreader and my own parser
    ...
    public ArrayList<Element> parse(Process process, AdaptorOptions options, File file)
    {
        ...
        Thread stderr;
        final CppCheckParser parser = new CppCheckParser(getProperties(), file, getAdaptorName());
    ...
    /**
     * get the version from the tool. To do this, the tool is executed with the
     * version option and the version number is parsed from its output.
     */
    @Override
    public String getGeneratorVersion()
    {
        final String[] commands = { "cppcheck", "--version" };
