...
public class CppCheckParser extends DefaultHandler
{
     ...
     * @param file
     */
    public CppCheckParser(Properties properties, File file, String name)
    {
        this.file = file;
        props = properties;
        findingCreator = new FindingCreator(properties, name);
    }
    
    /**
     * This gets called every time there is an xml element found. The idea here
     * is to extract the required elements, such as the error id, message, line
     * number, and file. This information should be passed on to the
     * FindingCreator which will then construct the entities in the toif output.
     */
    @Override
