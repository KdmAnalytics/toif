...
public class CoverageClaimGenerator {
   ...
   * @return
   */
  private static Collection<? extends Rule> getRules(ObjectFactory objFactory, Value cwe,
    ...
    try {
      ...
      String adaptorQuery = "SELECT ?descriptionText WHERE {"
          + "?finding <http://toif/toif:FindingHasCWEIdentifier> <" + cwe + "> . "
   ...
   * @return
   */
  private Collection<Claim> createClaims(ObjectFactory objFactory, Value segment,
      Repository repository) {
    ...
    try {
      ...
      String adaptorQuery = "SELECT ?cwe ?cweName WHERE { " + "<" + segment
          + "> <http://toif/contains> ?cwe ." + "?cwe <http://toif/type> \"toif:CWEIdentifier\" . "
          + "?cwe <http://toif/name> ?cweName . }";
   ...
   * @return
   */
  private List<CWECoverageClaimType> createCoverageClaims(ObjectFactory objFactory,
      Repository repository) {
    ...
    try {
      ...
      String adaptorQuery = "SELECT ?generatorName ?segment ?version WHERE { ?y <http://toif/path> ?x . ?segment <http://toif/contains> ?y . ?segment <http://toif/toif:TOIFSegmentIsGeneratedByGenerator> ?generator . "
          + "?generator <http://toif/name> ?generatorName . "
          + "?generator <http://toif/version> ?version}";
  ...
  private String getAFileFromSegment(Value segment, Repository repository) {
    ...
    try {
      ...
      String adaptorQuery = "SELECT ?fileName WHERE { <" + segment
          + "> <http://toif/contains> ?file . " + "?file <http://toif/type> \"toif:File\" ."
          + "?file <http://toif/name> ?fileName . }";
  ...
  private XMLGregorianCalendar getDate(Value segment, Repository repository) {
    ...
    try {
      ...
      String adaptorQuery = "SELECT ?dateValue WHERE { <" + segment
          + "> <http://toif/contains> ?date . " + "?date <http://toif/date> ?dateValue . }";
      ...
      while (queryResult.hasNext()) {
        ...
        try {
          ...
          xmlGrogerianCalendar.setTime(DatatypeConstants.FIELD_UNDEFINED,
  ...
  private String getLanguage(String fileName) {
    if (fileName.endsWith(".C")) {
      ...
      return "C++";
    } else if (fileName.endsWith(".Class")) { return "Java"; }
    if (fileName.endsWith(".c")) {
      return "C";
    } else if (fileName.endsWith(".java")) {
      return "Java";
  ...
  private String getLanguageType(String fileName) {
    if (fileName.endsWith(".c")) {
      ...
      return "Source Code";
    } else if (fileName.endsWith(".class")) { return "Byte Code"; }
    if (fileName.endsWith(".C")) {
      return "Source Code";
    } else if (fileName.endsWith(".Java")) {
      return "Source Code";
   ...
   * @throws IOException
   */
  private void writeIndividualClaimsToWriter(FileWriter writer,
      CWECoverageClaimType cweCoverageClaimType) throws IOException {
   ...
   * @throws IOException
   */
  private void writeOverallCoverageClaimsToWriter(FileWriter writer,
      List<CWECoverageClaimType> overallClaims) throws IOException {
