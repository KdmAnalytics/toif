...
findingCount3=$(find ${outDir} -name "*.xml" | xargs grep "GC_UNCHECKED_TYPE_IN_GENERIC_CALL" | wc -l)
findingCount4=$(find ${outDir} -name "*.xml" | xargs grep "ICAST_BAD_SHIFT_AMOUNT" | wc -l)
findingCount5=$(find ${outDir} -name "*.xml" | xargs grep "DMI_ANNOTATION_IS_NOT_VISIBLE_TO_REFLECTION" | wc -l)
findingCount6=$(find ${outDir} -name "*.xml" | xargs grep "DMI_FUTILE_ATTEMPT_TO_CHANGE_MAXPOOL_SIZE_OF_SCHEDULED_THREAD_POOL_EXECUTOR" | wc -l)
findingCount7=$(find ${outDir} -name "*.xml" | xargs grep "RV_ABSOLUTE_VALUE_OF_RANDOM_INT" | wc -l)
findingCount8=$(find ${outDir} -name "*.xml" | xargs grep "JLM_JSR166_UTILCONCURRENT_MONITORENTER" | wc -l)
findingCount9=$(find ${outDir} -name "*.xml" | xargs grep "JML_JSR166_CALLING_WAIT_RATHER_THAN_AWAIT" | wc -l)
findingCount10=$(find ${outDir} -name "*.xml" | xargs grep "DM_GC" | wc -l)
findingCount11=$(find ${outDir} -name "*.xml" | xargs grep "JAXWS_ENDPOINT" | wc -l)
findingCount12=$(find ${outDir} -name "*.xml" | xargs grep "JAXRS_ENDPOINT" | wc -l)
findingCount13=$(find ${outDir} -name "*.xml" | xargs grep "TAPESTRY_ENDPOINT" | wc -l)
findingCount14=$(find ${outDir} -name "*.xml" | xargs grep "WICKET_ENDPOINT" | wc -l)
findingCount15=$(find ${outDir} -name "*.xml" | xargs grep "STRUTS1_ENDPOINT" | wc -l)
findingCount16=$(find ${outDir} -name "*.xml" | xargs grep "STRUTS2_ENDPOINT" | wc -l)
findingCount17=$(find ${outDir} -name "*.xml" | xargs grep "SPRING_ENDPOINT" | wc -l)
findingCount18=$(find ${outDir} -name "*.xml" | xargs grep "ECB_MODE" | wc -l)

# The result should be 457
...
echo "Total number of TAPESTRY_ENDPOINT findings in the analyzed file:${findingCount13}"
echo "Total number of WICKET_ENDPOINT findings in the analyzed file:${findingCount14}"
echo "Total number of STRUTS1_ENDPOINT findings in the analyzed file:${findingCount15}"
echo "Total number of STRUTS2_ENDPOINT findings in the analyzed file:${findingCount16}"
echo "Total number of SPRING_ENDPOINT findings in the analyzed file:${findingCount17}"
echo "Total number of ECB_MODE findings in the analyzed file:${findingCount18}"
