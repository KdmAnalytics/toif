...
public class RatsAdaptor extends AbstractAdaptor
{
    ...
    {
        final String[] commands = { "rats", "--xml", "--quiet", options.getInputFile().toString() };
        return commands;
    }
    ...
    public ArrayList<Element> parse(Process process, AdaptorOptions options, File file)
    {
        ...
        Thread stderr;
        final RatsParser parser = new RatsParser(getProperties(), file, getAdaptorName());
    ...
    {
        final String[] commands = { "rats", "-help" };
        ...
        try
        {
            ...
            while ((strLine = br.readLine()) != null)
            {
                ...
                {
                    System.err.println(getAdaptorName() + ": Generator " + stringArray[1] + " found, only version v2.3 has been tested");
                    return stringArray[1].trim();
                }
            }
